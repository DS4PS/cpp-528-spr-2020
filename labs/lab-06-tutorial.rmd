...
```{r}
nmtc <- read.csv( "data/raw/NMTC/nmtc-sheet-01.csv", stringsAsFactors=F )
head( nmtc ) 

lihtc <- read.csv( "data/raw/LIHTC/LIHTCPUB.csv", stringsAsFactors=F )
head( lihtc )
...
Logs turn differences in levels to growth rates: 
...
log(C2) - log(C1) = default growth rate / secular market trend 

log(T1) - log(T1) = actual growth rate
...
The important coefficient, B3, would then represent the growth rate of the treatment group above the secular growth rate. 
...
Recall that you can still add control variable to the model. They just have to be measured over both time periods. 
...
Report and interpret the main results from the models. 
